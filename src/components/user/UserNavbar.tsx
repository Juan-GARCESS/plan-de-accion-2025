    ...
    <nav style={{ backgroundColor: '#ffffff', borderBottom: '1px solid #e5e7eb', boxShadow: '0 1px 2px rgba(0,0,0,0.04)', position: 'sticky', top: 0, zIndex: 1000 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontWeight: 800, fontSize: 20, marginLeft: 8 }}>
            Plan de acción{areaName ? ` - ${areaName}` : ''}
          </span>
        </div>
        <div style={{ position: 'relative' }} ref={menuRef}>
          ...
          {menuOpen && (
            <div style={{ position: 'absolute', right: 0, top: 52, width: 240, background: '#fff', borderRadius: 14, boxShadow: '0 12px 24px rgba(0,0,0,0.12)' }}>
              ...
              <div style={{ padding: 8 }}>
                <button onClick={() => router.push('/dashboard')} style={{ width: '100%', textAlign: 'left', padding: '10px 12px', borderRadius: 8, border: 'none', background: '#fff', cursor: 'pointer', fontWeight: 600, marginBottom: 4 }} onMouseEnter={(e)=>e.currentTarget.style.backgroundColor='#f3f4f6'} onMouseLeave={(e)=>e.currentTarget.style.backgroundColor='#fff'}>
                  Ver mi perfil
                </button>
